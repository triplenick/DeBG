...
BiRefNet models now use the same lower-memory CUDA options as BRIA. Restart DeBG after this change to release existing model sessions and apply the options. To benchmark BiRefNet with the cached model, run the DeBG venv Python with `tests/benchmark-bria.py birefnet-general`. Avoid benchmarking alongside another active inference job: they compete for GPU memory. The embedded server retains only one active inference session. Switching models releases the previous session before loading the next; cached renderer masks and finished images remain available. Repeated images with the same model reuse its session. Model load time and inference time are logged separately. To test switching with cached models, run `tests/benchmark-bria.py birefnet-general bria-rmbg birefnet-general` with the DeBG venv Python.
