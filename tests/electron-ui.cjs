...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const report = await win.webContents.executeJavaScript(`(async () => {
      ...
      const pasted=new DataTransfer(); pasted.items.add(new File([blob],'pasted.png',{type:'image/png'}));
      window.dispatchEvent(new ClipboardEvent('paste',{clipboardData:pasted,bubbles:true,cancelable:true}));
      await wait(()=>document.querySelectorAll('.card-done').length===3);
      Object.defineProperty(navigator,'clipboard',{configurable:true,value:{read:async()=>[{types:['image/png'],getType:async()=>blob}]}});
      document.querySelector('.paste-row button').click();
      await wait(()=>document.querySelectorAll('.card-done').length===4);
      const pasteCount=document.querySelectorAll('.card-done').length;
      const dock=document.querySelector('.process-dock').getBoundingClientRect();
      return {calls,maxActive,names,pasteCount,overflow:document.documentElement.scrollWidth>innerWidth,closed:[...document.querySelectorAll('details')].every(x=>!x.open),dockBottom:dock.bottom,height:innerHeight};
    })()`);
    assert.equal(report.calls,4);assert.equal(report.pasteCount,4);assert.equal(report.maxActive,1);
